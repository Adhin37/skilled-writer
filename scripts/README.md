...
| `health` | The toolkit's own wiring: skill frontmatter, uncited and dangling references, draft/audit cards against their dispatcher, `CLAUDE.md` section 3 against the directory listing, `optional:` toggles, orphan `novel.md` keys, every template accessor, documented commands against implemented ones, `.claude/commands/` against `CLAUDE.md` section 7, **every skill's `owns:` claim for completeness and exclusivity, and any pair of skills carrying the same 12-word passage**, line-number citations | no |
