...
of what is wrong, or when a report from `revision-pass` points at a symptom rather than a cause.
...
| The page is smooth and nothing pulls | no microtension; everyone feels one simple thing | `prose-quality` §Microtension — two feelings that do not fit, inside one person |
| The narrator explains what it all means | the theme is being stated instead of dramatized | `revision-pass` Pass 9d — a character may say it and be wrong; the narrator may not |
...
| Beats are all nods, shrugs and sighs | the default gesture set | `roles/draft/voice-separation.channels.md` — beats come from the hands field |
...
| Readers skim the openings | establishing paragraphs, scenery blocks | `world-texture` §2 and `roles/draft/world-texture.overbuilding.md` |
...
| Bad at it Tuesday, good at it Friday | the training montage | `roles/draft/competence-map.acquisition-ladder.md` — five stages, a named source per advance |
