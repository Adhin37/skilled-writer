...
| Names, terms, titles and spellings match `bible/lexicon.md` exactly — and so do its **style lines** (how numbers are written, forms of address, units) | two spellings of one name across forty chapters; "2 years" beside "two years" in a book whose lexicon spells small numbers out; a rank the lexicon gives to one person applied to another | fix the chapter. Propose a lexicon change under `For design:` only if the new form is genuinely better — a silent lexicon edit orphans every earlier chapter |
| A figure the bible prices means what the bible says it means | a price in days' wages used as a duration two chapters later | use the bible's meaning, or name the second meaning under `For design:` |
