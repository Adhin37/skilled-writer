...
5. **Step 5 — the state write.** The CCS block (with `gate>` condensed from the gate's line,
   `z4>` copied from `state/gate.md`, `cand>` and `gav>` copied from `state/brief.md`),
   `threads.md`, `growth.md`, `timeline.md`, `body.md` on a form change. Then stamp, last:
   `sw stamp ... --status revised --ledger`. `state/gate.md` is the gate's: copy from it, never
   edit it.
...
file and `z4>` from the gate's file, so none of them is written from memory; what only you can supply is
