...
other, when a scene is reading as an exchange of facts, or when Pass 2 says the dialogue is
correct and inert. It is a per-scene reference, not a per-line one: the decisions here are made
before the exchange is written.
...
At least one exchange per scene runs on one of them.
...
Subtext costs the reader attention, so it is spent, not sprinkled.
...
## A fifth technique: the third party

The four above run between two people. A third person in the room changes the arithmetic, and it
is the technique serials use most, because a serial cast is rarely alone: **two speakers who
understand each other, and one who must not.**

| | |
|---|---|
| how it works | the surface conversation is addressed to the room; the real one is addressed to one person in it. The excluded party is satisfied by the surface, so nothing has to be coded cleverly |
| what it needs | the reader must know what the two share *before* the scene, or there is nothing to overhear |
| what it buys | tension without confrontation — the scene can be dangerous while everyone is polite |
| the failure | the excluded party is a prop. Give them their own want, and let them nearly catch it |

*"We settled the Antwerp business last spring," he said, and passed her the salt. She had never
been to Antwerp. The clerk went on eating.*

## When subtext is the wrong tool

It is spent, not sprinkled, and four scenes make the same mistake:

- **A scene where nobody can afford it.** People under real pressure get blunt. A character who
  keeps being oblique while the building burns is a character the reader stops believing.
- **A scene the reader cannot yet decode.** Subtext between two people the reader cannot tell
  apart reads as confusion, not depth (`character-profile` — place them first).
- **The information scene.** Something the plot needs the reader to have, delivered obliquely,
  is a scene that has to happen twice.
- **Every scene.** A book in which nobody ever says the thing plainly is as flat as one in which
  everybody always does. The reserve exists to be spent.
