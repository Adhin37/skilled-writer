...
<!-- Chapter prose begins here.
     ...
     THE FOUR CHANNELS (see bible/lexicon.md — set once, never varied):
       "…"   speech
       '…'   direct thought, 1-3 per chapter, at decisions
       […]   system interfaces, in-world documents, narrative adjustments
       plain free indirect discourse — the default carrier of interiority, unmarked

     No headings inside the chapter body. No bold, no lists, no links. Scene breaks are a
     centered separator on its own line:

     * * *

     If pov.label_switches is true and this chapter changes POV, the first line is the POV
     character's name on its own line, italicized. Nothing else above the prose. -->
