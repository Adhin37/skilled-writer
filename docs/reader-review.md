...
|---|---|---|
| a fresh session or agent, no run context, not told it is a benchmark | **best** | preferred. Hand it the chapter paths and §1 of this file. Nothing else |
| a human who did not follow the run | **good** | as above |
...
Run #5's cold read, the worked example in §7, came from a reader who had not seen this file, the
...
its own right; §7 shows the shape.

---

## 7. Worked example — run #5, cold read, 2026-09-19
